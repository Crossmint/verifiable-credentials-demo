...
const Credential: React.FC<CredentialProps> = ({
  ...
  >(undefined);
  const [getEncCredential, setGetEncCredential] = useState<
    EncryptedVerifiableCredential | undefined
  >(undefined);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const isEncrypted =
    collection.metadata.credentialMetadata.encryption.type !== "none";
  ...
  const retrieveCredential = async () => {
    ...
    try {
      const credential = await credentialContext?.retrieve(
        collection,
        nft.tokenId,
        useDogemotoKey
      );
      if (isEncrypted && credential.credentialSubject == null) {
  ...
  return (
    <>
      <div
        key={key}
        className="flex rounded overflow-hidden shadow-lg bg-white"
      >
        ...
        <div className="px-6 py-4">
          ...
          <button
            ...
            onClick={() => {
              retrieveCredential().then(() => setIsModalOpen(true));
            }}
          >
            See Credential
          </button>
          ...
          <div className="bg-white rounded-lg p-8 max-w-lg w-full relative">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start">
              ...
              {isEncrypted && !getCredential ? (
                <p className="mb-4">
                  This credential is encrypted. Please decrypt it to view the
                  contents.
                </p>
              ) : (
                    ...
                    {isCourseNft() ? (
                      <div>
                        ...
                        <p className="mb-4">
                          ...
                          <br />
                          {new Date(
                            getCredential?.credentialSubject.issueDate
                          ).toDateString()}
                        </p>
            ...
            {isEncrypted && !getCredential ? (
              ...
              <div>
                {isValid !== undefined ? (
                  <div
                    className={`flex items-center max-w-32 mt-4 py-2 px-4 rounded ${
                      isValid
